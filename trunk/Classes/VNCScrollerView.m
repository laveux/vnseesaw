...
{
//NSLog(@"into handleTiltScrollTimer");
	int dxAutoScroll = 30, dyAutoScroll = 30;
	CGPoint ptLeftTop = [self bounds].origin;
...
//NSLog(@"Into tiltScroll method");
	
	if (!_eventFilter || ![[VNCPreferences sharedPreferences] useTiltScroll])
	{
		return;
	}
	...
	// Need to determine the orientation at change the values scroll*
	float scrollRight, scrollLeft, scrollUp, scrollDown;
	int orientationState = [_vncView getOrientationState];
	...
	switch (orientationState)
	{
		case kOrientationVerticalUpsideDown:
			if (accelX > scrollRight) {
				newAutoScroller = kAutoScrollerRight;
			} else if (accelX < scrollLeft) {
				newAutoScroller = kAutoScrollerLeft;
			}

			if (accelY < -scrollUp) {
				newAutoScroller |= kAutoScrollerDown;
			} else if (accelY > -scrollDown) {
				newAutoScroller |= kAutoScrollerUp;
			}
			break;
		...
		case kOrientationVertical:                                        
			...
			if (accelY < scrollDown) {
				newAutoScroller |= kAutoScrollerUp;
			} else if (accelY > scrollUp) {
				newAutoScroller |= kAutoScrollerDown;
			}
			break;
			
		case kOrientationHorizontalRight:
//NSLog(@"accel: x = %f, y = %f, z = %f",accelX,accelY,accelZ);
			if (accelX > scrollUp) {
				newAutoScroller = kAutoScrollerLeft;
			} else if (accelX < scrollDown) {
				newAutoScroller = kAutoScrollerRight;
			}

			if (accelY < scrollLeft) {
				newAutoScroller |= kAutoScrollerUp;
			} else if (accelY > scrollRight) {
				newAutoScroller |= kAutoScrollerDown;
			}
			break;
	...
	if (newAutoScroller != _currentAutoScrollerType)
	{
		...
		// Get rid of any old autoscroll timer.
		if (_tiltScrollTimer != nil)
		{
			[_tiltScrollTimer invalidate];
			[_tiltScrollTimer release];
			_tiltScrollTimer = nil;
		}
		
		// Setup autoscroll timer if we're scrolling.
		if (newAutoScroller != kAutoScrollerNone)
		{
			_tiltScrollTimer = [[NSTimer scheduledTimerWithTimeInterval:.1 target:self selector:@selector(handleTiltScrollTimer:) userInfo:nil repeats:YES] retain];
		}
	}
}
