...
//		controlOrigin = CGPointMake(200, 9);
		...
		// Use tilt scroll.
		UIPreferencesControlTableCell * useTiltScrollCell = [[UIPreferencesControlTableCell alloc] init];
		[useTiltScrollCell setTitle:NSLocalizedString(@"Use tilt scroll", nil)];
		
//		controlOrigin = CGPointMake(200, 9);
		_useTiltScrollSwitch = [[UISwitchControl alloc] init];
		[_useTiltScrollSwitch setOrigin:controlOrigin];
		[useTiltScrollCell setControl:_useTiltScrollSwitch];
		
		// Create array of cells.
		_cells = [[NSArray arrayWithObjects:mouseTracksCell, disconnectCell, showScrollIconCell, showZoomPercentCell, useTiltScrollCell, nil] retain];
	}
...
- (void)updateViewFromPreferences {
	...
	[_useTiltScrollSwitch setValue:BOOL_TO_SWITCH_VALUE([[VNCPreferences sharedPreferences] useTiltScroll])];
...
- (void)navigationBar:(id)navBar buttonClicked:(int)buttonIndex {
	switch (buttonIndex) {
		...
		case kServerListButton: {
			...
			[[VNCPreferences sharedPreferences] setUseTiltScroll:SWITCH_VALUE_TO_BOOL(_useTiltScrollSwitch)];
			break;
		}
	}
