...
- (void)sendPageUpKey:(id)sender {
	[_connection sendPageUpKey];
}

- (void)sendPageDownKey:(id)sender {
	[_connection sendPageDownKey];
}

- (void)sendHomeKey:(id)sender {
	[_connection sendHomeKey];
}

- (void)sendEndKey:(id)sender {
	[_connection sendEndKey];
}

- (void)sendRightArrowKey:(id)sender {
//NSLog(@"VNCView: sendRightArrowKey");
	[_connection sendRightArrowKey];
}

- (void)sendLeftArrowKey:(id)sender {
//NSLog(@"VNCView: sendLeftArrowKey");
	[_connection sendLeftArrowKey];
}

- (void)sendUpArrowKey:(id)sender {
//NSLog(@"VNCView: sendUpArrowKey");
	[_connection sendUpArrowKey];
}

- (void)sendDownArrowKey:(id)sender {
//NSLog(@"VNCView: sendDownArrowKey");
	[_connection sendDownArrowKey];
}
...
- (id)initWithFrame:(CGRect)frame {
	if ([super initWithFrame:frame]) {
		...
		[_keyboardView setPreferredKeyboardType: 0/*kUIKeyboardLayoutAlphabetTransparent*/];

		// Create arrows.
		CGSize defaultArrowSize = CGSizeMake(150, 150);
		subframe.origin = CGPointMake(0, frame.size.height - kControlsBarHeight - defaultArrowSize.height);
		subframe.size = defaultArrowSize;
		...
		_areArrowsVisible = NO;
		_areScrollersVisible = NO;
...
- (void)layoutControlsBar {
	...
	_controlsView = [[UIView alloc] initWithFrame:subframe];
	[_controlsView setOpaque:NO];
	float red[4] = {1, 0, 0, 0.0};
	[_controlsView setBackgroundColor:CGColorCreate(CGColorSpaceCreateDeviceRGB(), red)];
	...
	_arrowButton = [[UINavBarButton alloc] initWithImage:[UIImage imageNamed:@"arrows_icon.png"]];
	[_arrowButton setFrame:subframe];
	[_arrowButton setNavBarButtonStyle:0];
	[_arrowButton addTarget:self action:@selector(toggleArrows:) forEvents:kUIControlEventMouseUpInside];
	...
	subframe = CGRectMake(subframe.origin.x + kModifierKeyButtonWidth + 5 , (kControlsBarHeight - kControlsBarButtonHeight) / 2.0f + 1.0f, 40, kControlsBarButtonHeight);
	...
	[_controlsView addSubview:_arrowButton];
...
- (void)showControls:(bool)show {
	if (_areControlsVisible != show) {
		...
		if (_areControlsVisible) {
			...
			// Hide the arrow keypad if it was in view.
			if (_areArrowsVisible) {
				[self toggleArrows:nil];
			}
...
//NSLog(@"into topLeftVisiblePt");
}

- (void)tiltScrollgx:(float)accelX gy:(float)accelY gz:(float)accelZ {
	[_scroller tiltScrollgx:accelX gy:accelY gz:accelZ];
}
...
//NSLog(@"into changeViewPinnedToPoint");
}
...
//	scaleSpecialTypes wScaleState = [self getScaleState]
	scaleSpecialTypes wScaleThisButton;
...
//! The toggle arrow button has been pressed. This method assumes
//! the controls bar is visible.
- (void)toggleArrows:(id)sender {
//	NSLog(@"toggling keyboard: old=%d", (int)_isKeyboardVisible);
	
	if (_areArrowsVisible) {
		// Remove the arrow window.
		[_arrowView removeFromSuperview];
	} else {
		// Add in the arrow window.
...
//	bool notViewOnly = !isViewOnly;
	...
	if (isViewOnly) {
		...
		[_delegate setStatusBarMode: 1/*kUIStatusBarBlack*/ duration:0];
		} else {
		[_controlsView addSubview: _keyboardButton];
		[_controlsView addSubview: _shiftButton];
		[_controlsView addSubview: _commandButton];
		...
		[_controlsView addSubview: _controlButton];
		[_controlsView addSubview: _rightMouseButton];
		[_delegate setStatusBarMode: 0/*kUIStatusBarWhite*/ duration:0];
		}
/*	
...
- (void)setOrientation:(int)wOrientation bForce:(int)bForce {
...
syslog(LOG_DEBUG,"VNC Screen Size  = %f %f", vncScreenSize.width, vncScreenSize.height);

	if (bForce || (_connection && wOrientation != [_screenView getOrientationState])) {
		int oldOrientation = [_screenView getOrientationState];
